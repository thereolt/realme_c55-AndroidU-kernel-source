...
struct oplus_cpa {
	...
	struct work_struct update_chg_type_work;
...
static void oplus_cpa_update_chg_type_work(struct work_struct *work)
{
	struct oplus_cpa *cpa =
		container_of(work, struct oplus_cpa, update_chg_type_work);
	struct mms_msg *msg;
	int rc;
...
static int oplus_cpa_probe(struct platform_device *pdev)
{
	...
	INIT_WORK(&cpa->update_chg_type_work, oplus_cpa_update_chg_type_work);
...
int oplus_cpa_switch_end(struct oplus_mms *topic, enum oplus_chg_protocol_type type)
{
	...
	schedule_work(&cpa->update_chg_type_work);
